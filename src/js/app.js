  ...
  init: async function() {
    $.getJSON('../spots.json', function(data) {
      ...
      for (i = 0; i < data.length; i ++) {
        ...
        spotTemplate.find('.spot-distance').text(data[i].distance);
        spotTemplate.find('.spot-price').text(data[i].price);
        spotTemplate.find('.btn-book').attr('data-id', data[i].id);

        spotRow.append(spotTemplate.html());
      }
    });
    ...
    App.contracts.Adoption.deployed().then(function(instance) {
      ...
      for (i = 0; i < adopters.length; i++) {
        if (adopters[i] !== '0x0000000000000000000000000000000000000000') {
          $('.panel-spot').eq(i).find('button').text('Booked').attr('disabled', true);
        }
      }
    }).catch(function(err) {
